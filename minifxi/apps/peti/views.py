...
class PetListar(View): 
	...
	def get(self,request,*args,**kwargs):
		...
		if clie == '0':
			cursor.execute("CALL sp_pet_x_fecha ("+str(fecha_desde)+","+str(fecha_hasta)+")")   # ---> grcl456
		else:
			cursor.execute("CALL sp_pet_x_fecha_1 ("+str(fecha_desde)+","+str(fecha_hasta)+","+str(clie)+")")  # ---> grcl456
